...
def trans_sdf_to_vox(sdf, vox_thres=0.033, grid_to_gt_res=None, grid_affine=None, grid_scale=None, align_corners=True,
                     pad_mode='border'):
    mode = 'bilinear'
    sdf_s = sdf.clone()
    sdf_s = torch.nn.functional.grid_sample(sdf_s, grid_affine, mode=mode, align_corners=align_corners,
                                            padding_mode=pad_mode)
    sdf_s = torch.nn.functional.grid_sample(sdf_s, grid_scale, mode=mode, align_corners=align_corners,
                                            padding_mode=pad_mode)
    sdf_s = torch.nn.functional.grid_sample(sdf_s, grid_to_gt_res, mode=mode, align_corners=align_corners,
                                            padding_mode=pad_mode)
...
def trans_bin_sdf_to_vox(sdf, vox_thres=0.033, grid_to_gt_res=None, grid_affine=None, grid_scale=None,
                         align_corners=True, pad_mode='border'):
    mode = 'bilinear'
    sdf_s = sdf.clone()
    sdf_s = torch.nn.functional.grid_sample(sdf_s, grid_affine, mode=mode, align_corners=align_corners,
                                            padding_mode=pad_mode)
...
def rotate_mesh_360(mesh_renderer, mesh, dg=36):
    ...
    for i in range(dg):
        ...
        img = render_mesh(mesh_renderer, cur_mesh,
                          norm=False)  # b c h w # important!! no norm here or they will not align
