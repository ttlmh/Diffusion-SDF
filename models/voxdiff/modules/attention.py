...
from models.voxdiff.modules.diffusionmodules.util import checkpoint
...
class BasicTransformerBlock_Ucond(nn.Module):
...
class SpatialTransformer(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head,
                 depth=1, dropout=0., context_dim=None, pos_enc=0):
        super().__init__()
        self.in_channels = in_channels
        inner_dim = n_heads * d_head
        self.norm = Normalize(in_channels)
        self.context_dim = context_dim
        self.pos_enc = (pos_enc > 0)
        ...
        self.proj_in = nn.Conv3d(in_channels,
                                 ...
                                 padding=0)
        if self.pos_enc:
            self.pos_embedding = nn.Parameter(torch.randn(1, pos_enc))
        ...
        if context_dim > 0:
            ...
            self.transformer_blocks = nn.ModuleList(
                [BasicTransformerBlock_Ucond(inner_dim, n_heads, d_head, dropout=dropout)
                 for d in range(depth)]
            )
